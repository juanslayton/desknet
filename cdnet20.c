...
Author's address as of 7.26.16 is 260 Sundance Ct, Azusa, CA 91792. */
...
int Max_page = 0;
 ...
 /* Temporary for back compatibility.  Remove when ready. */
int Net_size;
...
int Tens_register = 0;
 ...
 /* Used to set session length in minutes. */
time_t Time = 0;
...
int Addend2[41];
 ...
 /* This array records how many 
points a student receives for a right 
...
int mod1[21];  /*  Stores division remainders.  */
int mod2[21];
int mod_flag[21];  /*  Value 1 indicates remainder is being entered.  */
...
int Divpoints[41]; 
 ...
 /* This flag shows that student is working with dollars. */
int Dollar[41];
...
int Expression1[41];  /*  Holds 1st term of student problems in Check Your Math */
int Expression2[41];  /*  Holds 2nd term of student problems in Check Your Math */
int Expression3[41];  /*  Holds correct answers for Check Your Math */
int Expression4[41];  /*  Holds student answers for Check Your Math  */
...
struct SDL_Rect line[42];  // Used to lay out 
struct SDL_Rect tagline[42];  // Used to lay out 
  ...
  for (e = 1; e <= 20; e++)
    {
      ...
      for (c = 0; c < 20; c++)
        {
          scratch = TTF_RenderText_Solid(fnt55, &Last_line[e][c], hue[e]);
        ...
        { 
          if (Goofed[e] == 1)
            {
              if (time(t) - Clk[e] > 3) /* END OR CONTINUE TIMEOUT PENALTY. */
                {
                  Goofed[e] = 0;
                  scratch = TTF_RenderText_Solid(fnt55, "XX", white);
                  SDL_BlitSurface(scratch, NULL, glass, &tilem[e][9]);
                  SDL_FreeSurface(scratch);
                  SDL_UpdateWindowSurface(windowframe);
                }
              ...
              if (Placeflag[e] == 1)
                 {
                   ...
                   scratch = TTF_RenderText_Solid(fnt55, &Z[0], hue[e]);
                   SDL_BlitSurface(scratch, NULL, glass, &tilem[e][9]);
                   SDL_FreeSurface(scratch);
                   SDL_UpdateWindowSurface(windowframe);
          ...
          if (Stuans[e] == Answer[e])
            { 
               for (c = 0; c < 20; c++)
                 {
                   scratch = TTF_RenderText_Solid(fnt55, &Last_line[e][c], white);
                   SDL_BlitSurface(scratch, NULL, glass, &tilem[e][c]);
                   SDL_FreeSurface(scratch);
                 }
               SDL_UpdateWindowSurface(windowframe);
               ...
               Last_line[e][11] = 32;
               Last_line[e][13] = (char) Tens_register + 48;
               Last_line[e][14] = (char) Ones_register + 48;
               if(Last_line[e][13] == 48){Last_line[e][13] = 32;}
               ...
               if(Score[e] < Win_score)
                 {
                   ...
                   for (c = 0; c < 20; c++)
                     {
                       if(20==40) scratch = TTF_RenderText_Solid(fnt33, &Last_line[e][c], hue[e]);
                       else scratch = TTF_RenderText_Solid(fnt55, &Last_line[e][c], hue[e]);
                       SDL_BlitSurface(scratch, NULL, glass, &tilem[e][c]);
                       SDL_FreeSurface(scratch);
                     }
          ...
          else if(Stuans[e] != Answer[e])
            {
              ...
              Z[0] = Last_line[e][9];              
              scratch = TTF_RenderText_Solid(fnt55, &Z[0], white);
              SDL_BlitSurface(scratch, NULL, glass, &tilem[e][9]);
              ...
              scratch = TTF_RenderText_Solid(fnt55, "XX" , black);
              SDL_BlitSurface(scratch, NULL, glass, &tilem[e][9]);
              SDL_FreeSurface(scratch);
              SDL_UpdateWindowSurface(windowframe);
            }
          ...
          if (Goofed[e] == 0)  /* GET STUDENT INPUT. */
            {
              ...
              if (Stuans[e] == Answer[e])
               ...
               else if(Last_line[e][11] != Answer[e])
                 {
                   ...
                   Z[0] = 'X';
                   Last_line[e][10] = 'X';
                   scratch = TTF_RenderText_Solid(fnt55, &Z[0], black);
                   SDL_BlitSurface(scratch, NULL, glass, &tilem[e][10]);
                   SDL_FreeSurface(scratch);
                   SDL_UpdateWindowSurface(windowframe);
                 }
...
main(int ARGC, char *ARGV[])
{
  ...
  fnt55 = TTF_OpenFont( "/usr/local/etc/fonts/ttf-bitstream-vera-1.10/VeraMono.ttf", 55 );
     ...
     for (e = 1;  e < 6; e++)
        {
          ...
          for (b = 0; b < 20; b++)
             {
               tile[e][b].x = (b * 24) + 5;
               tile[e][b].y = (c * 60)+50;  // y runs from 0 to 224
               
               tilem[e][b].x = (b * 33) + 5;
               tilem[e][b].y = (c * 60)+50;  // y runs from 0 to 224
             }
          line[e].x = 5 ;
          line[e].y = (c * 60)+50;  // y runs from 0 to 224
          line[e].w = 530;

          tagline[e].x = 120;
          tagline[e].y = (c * 60)+50;  // y runs from 0 to 224
          tagline[e].w = 405;
        }
          ...
          for (b = 0; b < 21; b++)
             {
               ...
               tile[e][b].y = (c * 60)+110;  // y runs from 280 to 579
                    
               tilem[e][b].x = (b * 33) + 5;  // x's run from 5 to 605
               tilem[e][b].y = (c * 60)+110;  // y runs from 280 to 579
             }
          line[e].x = 5;
          line[e].y = (c * 60) + 110;
          line[e].w = 530;
          ...
          for (b = 0; b < 21; b++)
             {
               ...
               tilem[e][b].x = (b * 33) + 525;  // x's run from 540 to 1015
               tilem[e][b].y = (c * 60) + 50;
             }
          ...
          for (b = 0; b < 21; b++)
             {
               ...
               tilem[e][b].x = (b * 33) + 525;
               tilem[e][b].y = (c * 60) + 110;
             }
          line[e].x = 540;
          line[e].y = (c * 60) + 110;
          line[e].w = 530;
  ...
  for (e = 1; e <= 20; e++)
    {
      ...
      Last_line[e][1] = (char) Factor1[e]+48;
      Last_line[e][3] = 'X';    
      Last_line[e][5] = (char) Factor2[e]+48;    
      Last_line[e][7] = '=';    
...
poll5()
{
  ...
  SDL_UpdateWindowSurface(windowframe);
              ...
              /* Ask each student to input problem. */
  for (e = 1; e <= 20; e++)
    {
...
/* # # # # # # # # # # # # # # # # # Step 0 # # # # # # # # # # # # # # # # # # # # */
          ...
          /* Assemble and print first term of problem. */
          if (Step[e] == 0)
            { 
              if(((a==10)||(a==42)||(a==43)||(a==45)||(a==47))&&Expression1[e]==0)
                {
                  start_over();
                  continue;
                }
...
//                }
              ...
              if(a==42||(a==43)||(a==45)||a==47) Step[e]=1;
            
              if((a>47)&&(a<58))
                {
                  Expression1[e] = (10*Expression1[e])+(a-48);                  
                
                  Place[e]++;
                   
                  /* If first term too big, start over. */
                  if (Place[e] > 7)            
                    {
                      start_over();     
                      continue;
                  ...
                  /* If replacing previous message, clear line. */
                  if (Cursor[e] == 0)
                    {
                      SDL_FillRect(glass, &line[e], SDL_MapRGB(glass->format,255,255,255));
                    }
...
/* # # # # # # # # # # # # # # # # # Step 3 # # # # # # # # # # # # # # # # # # # # */
                  ...
                  /* Assemble and Print Expression2 */
          ...
          if(Step[e]==3)
            {
              ...
              if((a>47)&&(a<58))
                {
                  Expression2[e]=(10*Expression2[e])+(a-48);
                
                  /* If second term too long, start over. */
                  if (Place[e] > 7)            
                    {
                      start_over();
                      continue;
                  ...
                  /* If replacing previous message, clear line, then print new character. */
                  if (Cursor[e] == 0)
                    {
                      SDL_FillRect(glass, &line[e], SDL_MapRGB(glass->format,255,255,255));
                      scratch=TTF_RenderText_Solid(fnt40, &Z[0], hue[e]);
                  ...
                  /* If not replacing previous message, just print new character. */
                  else  
                    {
                      scratch=TTF_RenderText_Solid(fnt40, &Z[0], hue[e]);
                      SDL_BlitSurface(scratch, NULL, glass, &tile[e][Cursor[e]]);
...
/* # # # # # # # # # # # # # # # # # Step 5 # # # # # # # # # # # # # # # # # # # # */
          ...
          /* Assemble and print student answer. */            
          if (Step[e]==5)
            {
              if(a==10) Step[e]=6;
 ...
 //                 if((Dollar[e]==1)&((Cursor[e]==0)&(a!=36)))
 //                   {
 //                     start_over();
 //                     continue;
 //                   }       
                  Expression4[e]=(10*Expression4[e])+(a-48);
...
/* # # # # # # # # # # # # # # # # # Step 6 # # # # # # # # # # # # # # # # # # # # */
          ...
          if (Step[e] == 6)
            {   
              /*  Calculate answers for appropriate operation.  */ 
              if (operationa[e] == 43) Expression3[e] = Expression1[e] + Expression2[e];
              if (operationa[e] == 42) Expression3[e] = Expression1[e] * Expression2[e];
              if (operationa[e] == 45)
                {
                  /* We don't do negative answers. */
                  if(Expression1[e]<Expression2[e])
                    {
                      start_over();
                      continue;
                    }
                  Expression3[e] = Expression1[e] - Expression2[e];
                }
              ...
              if (operationa[e] == 47)
                {
                  ...
                  Expression3[e] = Expression1[e] / Expression2[e];
                  mod2[e] = Expression1[e] % Expression2[e];
                }
               
              /*  Test for matching sums, differences, products, quotients.  */
              
              if (Expression3[e] != Expression4[e])
                {
                  SDL_FillRect(glass, &line[e], SDL_MapRGB(glass->format,255,255,255));
...
process()
{
  ...
  SDL_RenderClear(rends);
           ...
           /* COPY OPENING LINES OF 20 STUDENTS TO THE SCREEN. */
  for (e = 1; e <= 20; e++)
    {
      Cursor[e] = 0;
      for (a = 0; a < 20; a++)
...
{
  SDL_FillRect(glass, &line[e], SDL_MapRGB(glass->format,255,255,255));
  TTF_RenderText_Solid(fnt40, TTEXT[245], hue[e]);
  SDL_BlitSurface(scratch, NULL, glass, &line[e]);
  SDL_FreeSurface(scratch);
  SDL_UpdateWindowSurface(windowframe);
           ...
           if (Goofed[e] == 0)
             {
               ...
               if (Stuans[e] == Answer[e])
               ...
               else if(Last_line[e][11] != Answer[e])
                 {
                   ...
                   Last_line[e][10] = 'X';
                   scratch = TTF_RenderText_Solid(fnt55, &Last_line[e][10], black);
                   SDL_BlitSurface(scratch, NULL, glass, &tilem[e][10]);
                   SDL_FreeSurface(scratch);
                   SDL_UpdateWindowSurface(windowframe);
                 }

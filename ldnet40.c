...
int Tens_register = 0;
 ...
 /* Used to set available time for keyboarding (F) and word processing (G). */
int Time = 0;
...
int Tlineb[41]; 
 ...
 /* This array holds message numbers for T on menu. */
int Ttb[41];
...
void wedge(int);
 ...
 /* Set time limits for keyboarding and word processing sessions. */
void timeit();
...
struct SDL_Rect tagline[42];  // Used to assemble keyboarding messages.
  ...
  for (e = 1; e <= 40; e++)
    {
      Score[e]=0;
      Cursor[e] = 0;
      scratch = TTF_RenderText_Solid(fnt30, TTEXT[Tline[e]], hue[e]);
...
check()
{
  ...
  for (e = 1; e <= 40; e++)
    {
       Step[e] = 0;
       scratch = TTF_RenderText_Solid(fnt30, TTEXT[244], hue[e]);
       SDL_BlitSurface(scratch, NULL, glass, &line[e]);
       SDL_FreeSurface(scratch);
       Cursor[e] = 0;
       Page[e] = 0;
